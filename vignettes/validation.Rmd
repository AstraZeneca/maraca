...
3. `tte_data`: the time-to-event data
4. `scatter_data`: if plot was created with `density_plot_type = "scatter"` then contains 
dataset that was plotted in scatter plot, otherwise `NULL`  
5. `boxstat_data`: if plot was created with `density_plot_type = "box"` or
`density_plot_type = "default"` then contains the boxplot statistics, otherwise `NULL`
6. `violin_data`: if plot was created with `density_plot_type = "violin"` or
`density_plot_type = "default"` then contains the violin distribution data, otherwise `NULL`
7. `wo_stats`: if maraca object was created with `compute_win_odds = TRUE` then contains 
