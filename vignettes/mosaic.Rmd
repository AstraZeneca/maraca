...
for the active arm in the plot by using the flag `winning_prob = TRUE`.
```{r fig.width=7, fig.height=6}
...
mosaic_plot(maraca_with_win_odds, winning_prob = TRUE)
```

We can also add a diagonal line (highlighting even Win/Loss split) to compare
against  using the flag `diagonal_line = TRUE`
```{r fig.width=7, fig.height=6}
mosaic_plot(maraca_dat, diagonal_line = TRUE)
```
